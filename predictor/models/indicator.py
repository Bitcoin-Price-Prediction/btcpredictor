...
class IndicatorOracle:
    ...
    def __init__(self
        ...
        -------------
            ...
            days_for_training : int
                ...
                other words, prices that are "too old" are discarded from training as time
                goes on. This allows the oracle to adapt to new changes over time. If too
                many training days are specified, feature computation may take too long and
        ...
        if not IndicatorOracle._warmed_up:
            time.sleep(60 - datetime.datetime.now().second)
            IndicatorOracle._warmed_up = True
    ...
    def _loop(self, stop_event, verbose, show_progress, fig, window):
        ...
        while not self._stop_event.is_set():
            ...
            if verbose: self._log_msg('Computing features...')
            self._featuremtx = self._prepare(self._btc_actual)
            ...
            if self._lstmthread is None or (not self._lstmthread.is_alive() and (time.time() - last_updated) / 60 >= self._rfrsh_rate):
                target = prices[-trim_size:].to_numpy()
